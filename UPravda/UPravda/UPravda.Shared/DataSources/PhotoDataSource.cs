...
{
	public class PhotoDataSource : IDataSource
	{
		private static PhotoDataSource _dataSource = new PhotoDataSource();
		...
		public static async Task<PhotoDataSource> GetDataSourceAsync()
		{
		...
		public static async Task<PhotoDataSource> GetOfflineDataSourceAsync()
		{
		...
		{
			string xml = null;
			xml = await OfflineHelper.GetFromStorage(NewsType.Photo);
			...
			this._offlineNewsData.NewsType = NewsType.Photo;
		}
		...
		private async Task LoadingOnlineData()
		{
			...
			request = await a.GetAsync(new Uri("http://www.pravda.com.ua/rssiphone/mode_photos/?language=" + str));
			...
			var data = await Parse(xml);
			this._onlineNewsData = new News() { NewsItems = new ObservableCollection<NewsItem>(data), NewsType = NewsType.Photo };
			int j = 0;
		...
		private static async Task<IEnumerable<NewsItem>> Parse(string xml)
		{
			...
			await OfflineHelper.SaveToStorage(NewsType.Photo, xml);
		...
		async public Task GetPhotoGallery(NewsItem newsItem)
		{
